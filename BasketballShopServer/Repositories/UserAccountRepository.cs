...
namespace BasketballShopServer.Repositories
{
    public class UserAccountRepository(AppDbContext appDbContext) : IUserAccount
    {
        ...
        {
            if (model is null) return new ServiceResponse(false, "Model is empty");
            ...
            var checkIfAdminIsCreated = await appDbContext.SystemRoles.FirstOrDefaultAsync(_ => _.Name!.ToLower().Equals("admin"));
        ...
        public async Task<ServiceResponse> Logout(string accessToken)
        {
            var tokenInfo = await appDbContext.TokenInfo
                .FirstOrDefaultAsync(_ => _.AccessToken == accessToken);

            if (tokenInfo == null)
            {
                return new ServiceResponse(false, "Invalid or expired token.");
            }

            appDbContext.TokenInfo.Remove(tokenInfo);
